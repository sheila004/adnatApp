...
<html lang="en" dir="ltr">
  ...
  <tr>
    ...
    if ($stmt = $con->prepare("SELECT s.id, u.name, s.shift_date, s.start, s.finish, s.break_length FROM shifts AS s INNER JOIN users AS u ON s.user_id=u.id WHERE u.organisation_id =? AND s.id = ?")) {
       $stmt->bind_param("ii",  $_SESSION['org_id'],$_GET['eid']);
        $stmt->execute();
        $result = $stmt->get_result();
...
<form class="" action="functions.php?eid=<?php echo $_GET['eid']; ?>" method="post">
  <tbody>
  <tr>
...
<td><input type="submit" name="update_shift" value="Update" required/></td>
<td></td>
  </tr>
